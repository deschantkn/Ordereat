...
// Server logic
class Server {
  constructor(config, http, https, fs, path, url, StringDecoder, controllers, helpers) {
    this.http = http;
    this.https = https;
    this.url = url;
    this.config = config;
    ...
    this.httpsServerOptions = {
      key: this.fs.readFileSync(this.path.join(__dirname, '/../https/key.pem')),
      cert: this.fs.readFileSync(this.path.join(__dirname, '/../https/cert.pem'))
    };
  }
  ...
  init() {
    ...
    this.httpServer = this.http.createServer((req, res) => this.handleRequest(req, res));
    this.httpsServer = this.https.createServer(this.httpsServerOptions, (req, res) => this.handleRequest(req, res));

    if (process.env.NODE_ENV === 'production') {
      this.httpsServer.listen(this.config.httpsPort, () => console.log('\x1b[34m%s\x1b[0m', `HTTP server listening on port ${this.config.httpPort} in ${this.config.envName} mode`));
    } else {
...
const server = new Server(config, http, https, fs, path, url, StringDecoder, controllers, helpers);
